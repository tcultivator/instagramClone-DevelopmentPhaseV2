...
app.post('/uploadStory', upload.single('image'), (req, res) => {
    ...
    cloudinary.uploader.upload(req.file.path, { resource_type: 'auto' }, (err, result) => {
        ...
        if (err) {
            ...
            console.log(username)
            console.log(userId)
            const query = 'INSERT INTO story (userId,username,secure_url,datePosted,reactions)VALUES(?,?,?,?)'
            db.query(query, [userId, username, result.secure_url, formattedDate,null], (err, result) => {
                if (err) {
